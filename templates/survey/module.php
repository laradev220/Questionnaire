                         ...
                         <div class="pb-6 border-b border-gray-100 last:border-0 last:pb-0">
                             <div class="flex flex-col md:flex-row md:items-start md:justify-between">
                                 <p class="font-medium text-gray-800 mb-4 md:mb-0 md:flex-1 md:pr-4 text-base"><span
                                         class="text-gray-500 text-sm mr-2 font-mono"><?php echo htmlspecialchars($id); ?></span><?php echo htmlspecialchars($question['text']); ?></p>
                                 ...
                                 <div class="flex flex-col md:flex-row md:space-x-3 md:space-y-0 space-y-2 md:min-w-max">
                                     ...
                                     <?php elseif ($question['type'] === 'multiple_choice'): ?>
                                         <?php foreach ($question['options'] as $option): ?>
                                             <label class="flex items-center cursor-pointer group">
                                                 <input type="radio" name="<?php echo htmlspecialchars($id); ?>" value="<?php echo $option['option_value']; ?>"
                                                     class="peer sr-only">
                                                 <div class="w-5 h-5 rounded-full border-2 border-gray-300 flex items-center justify-center peer-checked:bg-blue-500 peer-checked:border-blue-500 transition-all group-hover:border-blue-400">
                                                     <i class="fas fa-check text-white text-xs peer-checked:block hidden"></i>
                                                 </div>
                                                 <span class="ml-2 text-sm text-gray-700 peer-checked:text-blue-600 font-medium"><?php echo htmlspecialchars($option['option_text']); ?></span>
                                             </label>
                                         <?php endforeach; ?>
                                     <?php endif; ?>
                                 </div>
